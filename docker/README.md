...
**Telegram Bot** — Create a bot via @BotFather, copy the token to `.env`, optionally set `TELEGRAM_ALLOWED_USERS` to restrict access. After `docker compose up`, message your bot directly.
...
```bash
# 1. Get the files (clone the repo, or download just this folder)
...
|----------|---------|--------------|
| `LANGUAGE` | `en` | Interface and AI language: `zh` Chinese / `en` English / `ja` Japanese |
...
**Telegram 机器人** — 在 @BotFather 创建机器人，将 token 复制到 `.env`，可选设置 `TELEGRAM_ALLOWED_USERS` 限制访问。`docker compose up` 后直接给机器人发消息即可。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `LANGUAGE` | `en` | 界面和 AI 语言：`zh` 中文 / `en` 英文 / `ja` 日文 |
...
| `TELEGRAM_ALLOWED_USERS` | | 允许使用机器人的 Telegram 用户 ID，逗号分隔（留空 = 所有人） |
...
**Telegram Bot** — @BotFather でボットを作成し、トークンを `.env` にコピー。`TELEGRAM_ALLOWED_USERS` でアクセスを制限可能。`docker compose up` 後、ボットに直接メッセージを送信。
...
```bash
# 1. ファイルを取得（リポジトリをクローン、またはこのフォルダのみダウンロード）
git clone https://github.com/wangjiake/JKRiver.git
cd JKRiver/docker
...
| 設定項目 | デフォルト | 説明 |
|----------|-----------|------|
| `LANGUAGE` | `en` | UI と AI の言語：`zh` 中国語 / `en` 英語 / `ja` 日本語 |
...
| `TELEGRAM_ALLOWED_USERS` | | ボット使用を許可する Telegram ユーザー ID（カンマ区切り、空 = 全員） |
